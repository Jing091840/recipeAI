...
# 修正：为每个时间步生成数据
timesteps = 3  # 假设有3个时间步

all_users_scores = []
for timestep in range(1, timesteps + 1):  # 从1到timesteps迭代
    for user_id, user_pref in enumerate(users_preferences, start=1):
        score_maps = {
...
df_all_users_scores.to_csv('t_gru3_sql.csv', index=False, encoding='utf-8')
